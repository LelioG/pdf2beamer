...
def _seed_context_score(record: Any, role: str) -> float:
    ...
    if "introduction" in section:
        if role == "problem":
            ...
            score += 0.5
    if any(term in section for term in ("method", "global-local", "input unification")):
        score += 3.0 if role == "method" else 0.2
    if any(term in section for term in ("experiment", "result", "ablation", "flops", "robust")):
    ...
    role_terms = {
        ...
        "contribution": ("propose", "contribution", "framework", "relayformer"),
        "method": ("method", "attention", "glra", "relay", "global-local"),
