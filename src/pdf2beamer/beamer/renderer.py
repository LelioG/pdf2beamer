...
class BeamerRenderer:
    ...
    def _document(self, slide_ir: SlideIR, slides_tex: list[str]) -> str:
        ...
        lines.extend(
            [
                ...
                r"  \hbox{%",
                (
                    r"    \begin{beamercolorbox}"
                    r"[wd=.78\paperwidth,ht=2.5ex,dp=1.2ex,leftskip=2ex]"
                    r"{author in head/foot}%"
                ),
                r"      \usebeamerfont{author in head/foot}\insertshorttitle",
                r"    \end{beamercolorbox}%",
                (
                    r"    \begin{beamercolorbox}"
                    r"[wd=.22\paperwidth,ht=2.5ex,dp=1.2ex,center]"
                    r"{date in head/foot}%"
                ),
                (
                    r"      \usebeamerfont{date in head/foot}"
                    r"\insertframenumber/\inserttotalframenumber"
                ),
                r"    \end{beamercolorbox}%",
                r"  }%",
                r"}",
    ...
    def _two_column_slide(self, slide: Slide) -> str:
        ...
        for bullets in (left, right):
            lines.extend(
                [r"\column{0.48\textwidth}", r"\begin{itemize}", r"\setlength\itemsep{0.45em}"]
            )
    ...
    def _figure_left_slide(self, slide: Slide, output_dir: Path, warnings: list[str]) -> str:
        ...
        if visual is None:
            ...
            lines.extend(
                self._visual_lines(visual, output_dir, warnings, max_height=r"0.52\textheight")
            )
    ...
    def _figure_top_slide(self, slide: Slide, output_dir: Path, warnings: list[str]) -> str:
        ...
        if visual is None:
            ...
            lines.extend(
                self._visual_lines(visual, output_dir, warnings, max_height=r"0.28\textheight")
            )
    ...
    def _table_slide(self, slide: Slide, output_dir: Path, warnings: list[str]) -> str:
        ...
        if visual is not None:
            lines.extend(
                self._visual_lines(visual, output_dir, warnings, max_height=r"0.32\textheight")
            )
        lines.extend(self._bullet_lines(slide, itemsep="0.25em"))
        lines.extend([self._comments(slide), r"\end{frame}"])
        return "\n".join(lines)
        ...
        if visual.path:
            ...
            if full_path.exists():
                return [
                    r"\begin{center}",
                    (
                        rf"\includegraphics[width=0.96\linewidth,height={max_height},"
                        rf"keepaspectratio]{{\detokenize{{{str(display_path)}}}}}"
                    ),
    ...
    def _equation_lines(self, visual: SlideVisual) -> list[str]:
        ...
        caption = escape_latex_preserving_math(visual.caption or "")
        lines = [
            r"\begin{block}{Equation}",
            rf"\centering\small\texttt{{{content}}}",
            r"\end{block}",
        ]
        if caption:
