...
def build_deck_plan(
    ...
    target_count = max_slides or len(specs)
    if target_count > len(specs):
        specs = _insert_context_detail_specs(
            specs, contexts or [], budget=target_count - len(specs)
        )
    specs = _limit_specs_preserving_takeaway(specs, target_count)
...
def _base_specs(
    ...
    for role in _BASE_ROLE_ORDER[1:]:
        if any(_node_matches_role(node.type, role) for node in argument_graph.nodes):
            ...
            roles.append(role)
        elif role == SlideRole.LIMITATIONS and has_limitation:
            roles.append(role)
...
def _limit_specs_preserving_takeaway(
    specs: list[tuple[SlideRole, list[str], str | None]],
    target_count: int,
) -> list[tuple[SlideRole, list[str], str | None]]:
    if target_count <= 0 or len(specs) <= target_count:
        return specs
    limited = specs[:target_count]
    if any(role == SlideRole.TAKEAWAY for role, _, _ in limited):
        return limited
    takeaway = next((spec for spec in reversed(specs) if spec[0] == SlideRole.TAKEAWAY), None)
    if takeaway is None:
        return limited
    return [*limited[:-1], takeaway]
...
def _role_for_context(context: RerankResult) -> SlideRole:
    ...
    if any(
        term in joined
        for term in ("limitation", "degrade", "redundancy", "trade-off", "tradeoff", "hurt")
    ):
        return SlideRole.LIMITATIONS
    if any(
        term in joined for term in ("experimental setup", "implementation", "dataset", "training")
    ):
        return SlideRole.EXPERIMENTS
    if any(
        term in joined
        for term in ("key results", "result", "ablation", "benchmark", "performance", "f1")
    ):
        return SlideRole.RESULTS
    if any(term in joined for term in ("architecture", "model", "module", "algorithm", "system")):
