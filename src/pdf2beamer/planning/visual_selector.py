...
def _candidate_score(
    slide: Slide, candidate: VisualCandidate, contexts: list[RerankResult]
) -> float:
    slide_tokens = _tokens(
        " ".join([slide.title, slide.main_message, *[b.text for b in slide.bullets]])
    )
    ...
    if candidate.caption and len(candidate.caption.split()) >= 4:
        score += 0.15
    if (
        candidate.page_index is not None
        and candidate.page_index <= 1
        and slide.role in {"problem", "method"}
    ):
        score += 0.08
