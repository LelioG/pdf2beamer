...
_ROLE_KEYWORDS = {
    ...
    "contribution": ("propose", "contribution", "framework", "approach", "key idea"),
    "method": ("method", "architecture", "algorithm", "model", "implementation", "approach"),
...
def _context_role_score(context: RerankResult, node_type: str, keyword: str) -> tuple[float, int]:
    ...
    if node_type == "result" and any(
        item in section for item in ("result", "experiment", "evaluation", "ablation", "analysis")
    ):
        section_bonus += 0.45
