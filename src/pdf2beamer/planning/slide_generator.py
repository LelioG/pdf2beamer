...
def generate_slide_ir(
    ...
    slide_ir = _slide_ir_from_payload(
        payload, deck_plan, argument_graph, warnings, contexts or [], paper_ir
    )
...
def _slide_ir_from_payload(
    ...
    slide_ir = attach_relevant_visuals(SlideIR(slides=slides), paper_ir, contexts)
    slides = [_polish_slide(slide, argument_graph) for slide in slide_ir.slides]
    ...
    return SlideIR(
        paper_title=_clean_paper_title(
            _optional_str(payload.get("paper_title")) or argument_graph.paper_title
        ),
...
def _ground_bullets(
    bullets: list[SlideBullet], fallback_source_ids: list[str]
) -> list[SlideBullet]:
    ...
    return [
        ...
        else bullet.model_copy(update={"source_ids": fallback_source_ids})
        for bullet in bullets
    ]
...
def _sanitize_visuals(value: object, warnings: list[str]) -> list[SlideVisual]:
    ...
    for index, raw_visual in enumerate(value):
        ...
        if visual_type in {"figure", "image"} and not path:
            warnings.append(
                f"Removed incomplete visual at index {index}: figure/image has no path."
            )
            continue
...
def _polish_slide(slide: Slide, argument_graph: ArgumentGraph) -> Slide:
    ...
    for bullet in slide.bullets:
        if _looks_truncated(bullet.text):
            continue
        resolved = _resolve_orphan_reference_bullet(bullet.text)
        if resolved is None:
            continue
        bullets.append(bullet.model_copy(update={"text": resolved}))
    return slide.model_copy(update={"bullets": _dedupe_bullets(bullets)})


def _resolve_orphan_reference_bullet(text: str) -> str | None:
    cleaned = " ".join(text.split()).strip(" ,;:.-")
    cleaned = re.sub(
        r"\b(?:as shown|shown|reported|presented)\s+in\s+(?:table|figure)\s*\d+\b,?",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
    ...
    if re.search(r"\b(?:table|figure)\s*\d+\b", cleaned, flags=re.IGNORECASE):
        return None
    weak_reference_starts = (
        "the corresponding results",
        "corresponding results",
        "the corresponding examples",
        "corresponding examples",
    )
    if lowered.startswith(weak_reference_starts):
        return None
    return _polish_evidence_point(cleaned)
...
def _renumber_slides(slides: list[Slide]) -> list[Slide]:
    return [
        slide.model_copy(update={"id": f"slide_{index + 1:02d}"})
...
def _ensure_slide_content(
    ...
    if slide.bullets:
        ...
        bullets = _merge_bullets(context_bullets, bullets, _MIN_BULLETS_BY_ROLE.get(slide.role, 3))
        bullets = _enrich_bullets(slide.role, bullets, nodes)
        source_ids = list(
            dict.fromkeys(
                [*slide.source_ids, *(sid for bullet in bullets for sid in bullet.source_ids)]
            ),
        )
    ...
    bullets = _merge_bullets(context_bullets, [], _MIN_BULLETS_BY_ROLE.get(slide.role, 3))
    bullets = _enrich_bullets(slide.role, bullets, nodes)
    source_ids = list(
        dict.fromkeys(
            [*slide.source_ids, *(sid for bullet in bullets for sid in bullet.source_ids)]
...
def _enrich_bullets(
    ...
    for node in nodes:
        if _is_generic_fallback_text(node.text):
            continue
...
def _context_bullets_for_role(
    ...
    if not selected_contexts:
        selected_contexts = [
            context for context in contexts if _context_matches_role(role, context)
        ]
    if not selected_contexts:
...
def _context_matches_role(role: str, context: RerankResult) -> bool:
    ...
    haystack = " ".join((query, seed, section))
    role_terms = {
        "problem": ("problem", "introduction"),
        "contribution": ("contribution", "abstract"),
        "method": ("method", "approach"),
        "architecture": ("method", "architecture", "model"),
        "experiments": ("experimental setup", "experiment", "dataset"),
        "results": ("key results", "result", "evaluation", "benchmark"),
        "limitations": ("limitation", "caveat", "trade-off", "tradeoff"),
        "takeaway": ("conclusion", "takeaway"),
    }
    return any(term in haystack for term in role_terms.get(role, (role,)))
...
def _context_role_score(role: str, context: RerankResult) -> float:
    ...
    terms = {
        ...
        "contribution": ("contribution", "propose", "framework", "novel", "approach"),
        "method": ("method", "propose", "architecture", "algorithm", "model"),
        "architecture": ("architecture", "module", "submodule", "model", "system"),
        "experiments": (
            "experiment",
            "dataset",
            "benchmark",
    ...
    section_boosts = {
        ...
        "architecture": ("method", "architecture", "model", "system"),
        "experiments": ("experiment", "dataset", "implementation"),
        "results": ("result", "evaluation", "ablation", "benchmark"),
...
def _main_message_from_evidence(
    ...
    for node in nodes:
        if not _is_generic_fallback_text(node.text):
            return _compact_text(node.text, max_chars=150)
...
def _is_generic_fallback_text(text: str) -> bool:
    ...
    generic_starts = (
        "the paper addresses a scientific problem",
        "the paper contributes a method",
        "the method is summarized",
        "the results are summarized",
        "the takeaway is based",
    )
...
def _evidence_points(text: str) -> list[str]:
    ...
    compact = (
        compact.replace("vs. ", "vs ").replace("e.g. ", "for example ").replace("i.e. ", "that is ")
    )
    compact = re.sub(r"(?<=[A-Za-z)])and\b", " and", compact)
    compact = compact.replace("fixedsize", "fixed-size")
    raw_parts: list[str] = []
    ...
    for part in raw_parts:
        ...
        if not cleaned or len(cleaned.split()) < 4 or _looks_truncated(cleaned):
            continue
        point = _fit_evidence_point(cleaned, max_chars=90)
        if point is not None:
            points.append(point)
    return points


def _fit_evidence_point(text: str, max_chars: int) -> str | None:
    cleaned = _polish_evidence_point(" ".join(text.split()).strip(" ,;:.-"))
    if len(cleaned) <= max_chars:
        return cleaned if _usable_evidence_point(cleaned) else None

    candidates: list[str] = []
    for separator in (": ", ", ", " and ", " while ", " whereas ", " because ", " by ", " with "):
        parts = cleaned.split(separator)
        if len(parts) <= 1:
            continue
        prefix = _polish_evidence_point(parts[0].strip(" ,;:.-"))
        candidates.append(prefix)
        suffix = _polish_evidence_point(separator.join(parts[1:]).strip(" ,;:.-"))
        candidates.append(suffix)

    for candidate in candidates:
        if len(candidate) <= max_chars and _usable_evidence_point(candidate):
            return candidate
    return None


def _polish_evidence_point(text: str) -> str:
    if not text:
        return text
    first = text[0]
    if first.islower():
...
def _usable_evidence_point(text: str) -> bool:
    ...
    weak_starts = (
        ...
        "because ",
        "to address ",
        "for manipulated video sequences",
    )
    if lowered.startswith(weak_starts):
        return False
    words = text.split()
    return 4 <= len(words) <= 14 and len(text) <= 90 and not _looks_truncated(text)
...
def _compact_text(text: str, max_chars: int) -> str:
    ...
    # Prefer complete clauses. If no clean boundary exists, keep a hard limit.
    ...
    space = window.rfind(" ")
    if space >= max(48, int(max_chars * 0.6)):
...
def _looks_truncated(text: str) -> bool:
    ...
    if last.lower() in {
        ...
        return True
    allowed_short = {"F1", "F2", "AUC", "ROC", "n=1", "n=2", "n=3"}
    if last not in allowed_short and last.isalpha() and len(last) <= 3 and last.islower():
        return True
    if (
        last not in allowed_short
        and last.isalpha()
        and len(last) <= 4
        and not last.islower()
...
def _fallback_slide_ir(
    ...
    slide_ir = attach_relevant_visuals(SlideIR(slides=slides), paper_ir, contexts or [])
    slides = [_polish_slide(slide, argument_graph) for slide in slide_ir.slides]
...
def _fallback_slide_for_plan(
    ...
    context_bullets = _context_bullets_for_role(role, contexts or [], planned.target_evidence)
    bullets = _merge_bullets(context_bullets, [], _MIN_BULLETS_BY_ROLE.get(role, 3))
